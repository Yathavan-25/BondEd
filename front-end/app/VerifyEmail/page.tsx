    ...
    <div className="min-h-screen bg-[#000000] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(120,119,198,0.3),rgba(255,255,255,0))] flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md p-8 sm:p-10 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] flex flex-col items-center text-center text-white">
        ...
        <p className="text-white/80 font-medium mb-8 leading-relaxed">
          ...
          <br /><br />
          If you opened this on your phone, you can safely close this page and <strong className="text-white">return to your computer</strong> — it will automatically continue to the next step!
        </p>

        {/* If they happen to be logged in on this device, give them a button to continue */}
        {isLoggedIn ? (
          <button
            onClick={() => router.push(`/OnBoardingFlow/${auth.currentUser?.uid}`)}
            className="w-full flex items-center justify-center gap-2 py-4 px-6 rounded-xl bg-white text-gray-900 font-bold hover:bg-white/90 transition-all shadow-lg active:scale-[0.98]"
          >
          ...
          <button
            ...
            className="w-full py-4 px-6 rounded-xl border border-white/30 bg-white/10 font-semibold text-white hover:bg-white/20 transition-all"
          >
            Go to Login
          </button>
        )}
