...
import { 
  ...
  ArrowRight, Sparkles, Plus, Loader2, Mic, AlertCircle, Clock, Lightbulb, Brain, Target
} from "lucide-react";
import { 
...
export default function OnboardingFlow() {
  ...
  useEffect(() => {
    ...
    const onCallEnd = () => {
      ...
      setStep((currentStep) => {
        if (currentStep === 3) {
          alert("The connection was lost. Please try the exam again.");
          return 1; 
        }
        return currentStep;
      });
    ...
    const onMessage = async (msg: any) => {
      ...
      if (msg.type === "tool-calls" && msg.toolCallList?.[0]?.function?.name === "submitProfile") {
        isProfileCall = true;
        profileData = msg.toolCallList[0].function.arguments; 
      } else if (msg.type === "function-call" && msg.functionCall?.name === "submitProfile") {
        isProfileCall = true;
        profileData = msg.functionCall.parameters; 
      }

      if (isProfileCall && profileData) {
        setStep(4);
        vapi.stop(); 
        ...
        try {
          ...
          if (!token) {
            alert("Security Error: You are not logged in or your token expired.");
            setStep(1);
            return;
          }
          ...
          if (res.ok) {
            ...
            setTimeout(() => setStep(5), 2500); 
             ...
             alert("The server rejected your profile. Please check your authentication.");
             setStep(1); 
          }
        } catch (error) {
  ...
  const startVoiceExam = async () => {
    if (isConnected) vapi.stop();
      ...
      try {
        const subjectNames = selectedSubjects.map(id => VAST_SUBJECTS.find(s => s.id === id)?.label || id).join(", ");
        await vapi.start(process.env.NEXT_PUBLIC_VAPI_ASSISTANT_ID!, {
          variableValues: { 
            subjects: subjectNames, 
            topics: selectedTopics.join(", "),
            topicsBySubject: getTopicsBySubjectString()
          }
        });
      } catch (e) {
        console.error(e);
          ...
          <AnimatePresence mode="wait">
                  ...
                  <>
                    ...
                    <div className="flex gap-4">
                      ...
                      <button onClick={startVoiceExam} disabled={isConnecting} className="flex items-center gap-2 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-lg bg-blue-600 hover:bg-blue-700">
                        {isConnecting ? <><Loader2 className="size-10 animate-spin" /> Connecting...</> : <><Mic className="w-5 h-5" /> Start Voice Exam</>}
                      </button>
                    </div>
                  </>
                ) : (
            ...
            {step === 4 && (
              <motion.div key="s4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col items-center justify-center text-center">
                 <Loader2 className="size-10 text-violet-600 animate-spin mb-6" />
                   ...
                   <div className="bg-gray-50 p-6 rounded-[24px] border border-gray-100 flex flex-col items-center justify-center h-80">
                     <ResponsiveContainer width="100%" height="100%">
                       <RadarChart cx="50%" cy="50%" outerRadius="70%" data={[
                         { metric: "Extroversion", value: dynamicProfile.extroversionScore || 0 },
                 ...
                 <div className="flex flex-col gap-4">
                   ...
                   <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]">
                     <div className="flex items-center justify-between mb-4">
                       ...
                       <span className="px-3 py-1 bg-violet-50 text-violet-700 font-bold rounded-lg text-sm capitalize">{dynamicProfile.socialTemperament}</span>
                     </div>
                     <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                       <p className="text-sm text-gray-600 font-medium leading-relaxed">
