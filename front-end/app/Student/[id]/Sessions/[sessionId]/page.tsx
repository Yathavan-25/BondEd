...
import {
  ...
  Sparkles, Radio, Loader2, AlertTriangle, Clock
} from "lucide-react";
...
export default function ActiveSessionRoom() {
  ...
  // 1. Fetch Room URL AND Credits
  useEffect(() => {
    const fetchRoomAndCredits = async () => {
      try {
        onAuthStateChanged(auth, async(user) =>{
          if(user) {
            ...
            if (credRes.ok) {
                ...
                setAvailableMinutes(minutesLeft);
                
                // Block entry if out of credits
                if (minutesLeft <= 0) {
                    alert("You are out of Live Collaboration minutes! Please add more credits to join.");
    ...
    if (sessionId) fetchRoomAndCredits();
  }, [sessionId, studentId, router]);

  // 2. Init Daily
  useEffect(() => {
    if (!roomUrl || !callContainerRef.current || availableMinutes <= 0) return;
    ...
    callFrame.join({ url: roomUrl }).then(() => {
      try { callFrame.startTranscription(); } catch (err) {}
    });
  ...
  // 3. Real-Time Timer & Deduction Check
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (leftSec <= 0) {
      ...
      handleLeave(true); // Force kick out
    } else if (leftSec <= 120) {
  ...
  // Handle Safe Exit and Credit Deduction
        ...
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex min-w-0 items-center gap-3 md:gap-4">
            ...
            <div className="flex flex-col gap-1 min-w-0">
              <h1 className="truncate text-lg sm:text-xl font-bold text-slate-900 leading-tight">Active Study Session</h1>
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-gray-200 shadow-sm">
            <Clock className={`w-4 h-4 ${minutesRemaining <= 2 ? 'text-red-500 animate-pulse' : 'text-[#1363CB]'}`} />
            <span className={`text-sm font-bold ${minutesRemaining <= 2 ? 'text-red-600' : 'text-gray-700'}`}>{minutesRemaining}m left</span>
          </div>
        </header>
        ...
        <div className={`grid gap-6 transition-[grid-template-columns] duration-500 ease-in-out ${aiState !== "idle" ? "lg:grid-cols-[minmax(0,1fr)_380px]" : "grid-cols-1"}`}>
          ...
          <div className="flex flex-col gap-6 min-w-0">
            ...
            <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-4 sm:p-5">
              ...
              <div ref={transcriptScrollRef} className="px-2 py-2 space-y-4 max-h-75 overflow-y-auto scroll-smooth">
